...
| `weightKg`（有限性の根拠） | 列は `numeric(6,1)`（ADR-0022）で `Infinity` を格納できない。ただし Dart 側で非有限値が作られ得るため、関数側でも必ず判定する | 同上 |
...
| 対象 | 規則 | 根拠 |
|---|---|---|
| 内部値・画面に渡す `target_g` | 小数第1位で四捨五入（Dart: `(v * 10).round() / 10`） | Dart の `double` の丸め残差がそのまま画面に出るのを防ぐ。切り上げ/切り捨ては目標を過大/過小に見せるため使わない |
...
| 3 | 単位の一貫性 | `weight_kg` は kg、`protein_g` 系と `target_g` は g。どちらも `numeric(6,1)`（Dart は `double`）のため取り違えても型検査は通り、関数側でも検出できない（TC-FEAT07-12） | 🟡 中 |
...
- 論点6: 確定は「`protein_gauge` を null にして 200 を返す」（FEAT-05 §3 が正本・§4.3）。~~段3の契約改訂が要る。~~（**解決**・2026-08-08・段3 §4.3 に反映済み）
...
> ~~⚠️ 要確認（人間判断）: 段3との具体的な乖離。旧契約は次のように置き換わる。~~（**解決**・2026-08-08）
>
...
> - ~~段3の契約表とエラー節の改訂が要る~~
>
> **段3は改訂済み。** 3件とも `../../30_データ・IF設計/02_API設計.md §3` に置き換え後が載っている。
> RPC の契約本体は同 §4.3（`get_dashboard`）・§4.4（`get_protein_remaining`）。
> 旧 `/api/*` は同 §3 末尾の対比表に残るだけである。
> エラー節も同 §5.1・§5.4 で `PostgrestException` の SQLSTATE 写像に置き換わった。
...
> 契約の正本は FEAT-05 §3。~~段3（`../../30_データ・IF設計/02_API設計.md §4.3`）の改訂が要る。~~（**解決**・2026-08-08）
> **段3は改訂済み。** 同 §4.3 に「体重が未設定 → `protein_gauge` を `null` にして 200 を返す」がある。
> 同 §4.3 の戻り値サンプルにも「体重が未設定なら protein_gauge ごと null」と注記されている。
