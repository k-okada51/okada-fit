...
`weight_kg` の取得は**呼び出し側（FEAT-05 / FEAT-06 / FEAT-09）が行う**。
...
| 2 | 過去日の目標値 | `users.weight_kg` は現在値のみを保持し、過去日のダッシュボードも**現在の体重**で計算される。体重が変わると過去の達成率が遡って変わる。FEAT-06 の「体重の履歴を持たない」問題と同一根 | 🔴 高 |
...
- 論点2: 日次スナップショット（体重履歴テーブル、または集計時点の目標値の保存）が要るが、本PRでは追加しない。
...
> ⚠️ 要確認（人間判断）: #2 過去日の目標値。次のどちらを採るか。
>
> - 日次スナップショット（体重履歴テーブル、または集計時点の目標値保存）を持つ
> - 「過去分も現在の体重で再計算される」仕様を許容する
> - 前者はDBスキーマ追加を伴うため本PRでは判断しない
