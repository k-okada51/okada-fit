...
FEAT-05（ダッシュボード）・FEAT-07（必要タンパク質量算出）・FEAT-09（タンパク質残量と不足分提示）は本機能が書き込んだ `users.weight_kg` の読み手にあたる。
...
- ERRドメイン `ERR-PROFILE-*` は FEAT-07（必要タンパク質量算出）と共有する。FEAT-06 は **001〜019** のみを使う。
- **020以降は FEAT-07 に譲る。**
...
- 保存成功後に SCR-01（ダッシュボード）の取得結果を破棄して再取得する。
