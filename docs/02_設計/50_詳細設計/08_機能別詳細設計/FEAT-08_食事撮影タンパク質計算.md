...
| `eaten_date` | ISO 8601 の日付。未来日・1年以上前は不可 `[仮]` | Flutter | ERR-MEAL-007 (400) |
| `eaten_time` | ISO 8601 の時刻または null | Flutter | ERR-MEAL-007 (400) |
| `intake_count` | 整数かつ1以上、または null（DB CHECK と同値） | Flutter・DB CHECK | ERR-MEAL-006 (400) |
...
| `toMealLogRow(input)` | Dart | 保存行の組み立て | 栄養4項目＋`eaten_date`/`eaten_time`/`intake_count` のみを写す |
...
- 対処は表示上のものにとどめる。
- **脂質の入力欄にのみ**「揚げ物・炒め物では実際より少なく出る傾向があります」の注記を添える（§7）。
- 既定で手修正しやすい状態にする。
...
| ERR-MEAL-006 | 400 | 保存時の栄養4項目が欠落・非数値・負値 | 該当項目を指し示す | false | warn |
| ERR-MEAL-006 | 400 | `intake_count` が不正 | 同上 | false | warn |
...
| 成功（解析結果） | `food_name` を見出し、`dish_names` を `Chip` 群で表示 | 手修正・［記録する］可 |
| 同上 | 栄養4項目は `TextFormField`＋数値 `TextInputFormatter` | 同上 |
| 同上 | 初期値＝AI出力。手修正できる | 同上 |
| 同上 | 脂質欄に `helperText` で精度注記（§4） | 同上 |
...
| 同上 | 入力欄をリセットし、当日一覧と FEAT-09 の残量を再取得 | 同上 |
| エラー（AI系・送信系） | `ScaffoldMessenger.showSnackBar`（エラー色） | 再解析・手入力とも可 |
| 同上 | 結果領域に ERR-ID 由来のメッセージ | 同上 |
| 同上 | ［手入力で記録する］導線を併置（NFR-AVAIL-05） | 同上 |
...
- [AC] Given 解析結果が表示されている When 数値を手修正して［記録する］を押す Then 修正後の栄養4項目のみが1行保存され、写真は保存されない
...
| # | 論点 | 内容 | 重大度 |
|---|---|---|---|
| 1 | `eaten_date`/`eaten_time` を誰が決めるか未定義 | 段3 §4.1 に定義が無く `eaten_date` は not null。本書は §3.3 でクライアント送信 `[仮]`。サーバ時刻なら過去分を記録できず、端末時刻なら TZ・時計ずれが入るため、FEAT-05 と同規則に揃える | 🔴 高 |
| 2 | `intake_count` の業務的意味が未確定 | `01_データモデル.md` §8-6 の未解決。栄養4項目が1食分か `intake_count` 倍かで FEAT-09 の日次合計が変わる。本書は1食分として設計しており、乗算解釈なら FEAT-08/09 双方とUIに波及する | 🔴 高 |
| 3 | ⑦→⑧の間の手修正の可否 | §7 で手修正可としたが要件に明記が無い。不可なら脂質 MAPE 32.7%・家庭料理 18.8% の誤差が固定され、可なら §3.4 の保存時検証が必須。由来列が無く新列追加も禁止のため ADR-0001 の精度モニタリングが行えない | 🔴 高 |
...
| 7 | 脂質の精度をUIでどう扱うか（糖質 9.8% も n=10 の小標本） | 脂質 MAPE 32.7% は上位5モデル中で最悪（ADR-0001）。本書は §4/§7 の注記＋手修正可だが承認が要る。代替は (a) 非表示で0保存＝虚偽値、(b) レンジ表示＝保存不可、(c) Pro 昇格＝+1.7秒 | 🟡 中 |
...
> ⚠️ 要確認（人間判断）: #1 `eaten_date`/`eaten_time` の決定主体を確定してください。
> 選択肢はクライアント時刻／サーバ時刻／利用者入力の3つ。FEAT-05 の日付境界規則と同時に決めてください。
> `../../30_データ・IF設計/02_API設計.md` §4.1 への追記が必要です。

> ⚠️ 要確認（人間判断）: #2 `meal_logs.intake_count` の業務的意味を確定してください。
> 栄養4項目が1食分か `intake_count` 倍かで、FEAT-09 の合計算出式が変わります。

> ⚠️ 要確認（人間判断）: #3 解析結果と保存の間で栄養値を手修正できる仕様としてよいか承認してください。
...
> 選択肢は「注記＋手修正で許容する」か「モデルを Pro へ昇格する」の2つ。
