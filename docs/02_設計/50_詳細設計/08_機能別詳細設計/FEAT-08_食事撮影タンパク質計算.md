...
| モデル | 環境変数 `GEMINI_MODEL`（既定 `gemini-3.7-flash`） | ADR-0025 |
| 認証 | ヘッダ `x-goog-api-key: $GEMINI_API_KEY`。**環境変数のみ**に置く | NFR-SEC-02 |
| プロンプト | `contents[0].parts[0].text = MEAL_ANALYZE_PROMPT`。**本文は ADR-0026**（実体は `supabase/functions/analyze-meal/prompt.ts`） | ADR-0026 |
...
| 9 | ~~プロンプト文言が未確定~~（**解決**） | **2026-08-22 決定（ADR-0026）。断定「〜商品です」を外し、外食と家庭料理の両方を想定させる文にした。** 本文は `analyze-meal/prompt.ts`。⚠️ ADR-0001 の実測 MAPE は**旧文言のもの**であり、この文言では測っていない。再ベンチ（PoC 220件・約$0.5）は ADR-0026 の残課題 | — |
