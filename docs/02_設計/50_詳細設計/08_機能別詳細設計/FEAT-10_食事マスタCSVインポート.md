...
### 食品マスタの一覧・編集・削除（確定・2026-08-22）

**取り込んだ食品を1件ずつ直せるようにする。** CSV の再取込では直せないため。

| 要素 | 挙動 |
|---|---|
| 置き場所 | SCR-05（設定・プロフィール）。CSV取込のすぐ下 |
| 一覧 | `name` の昇順。**件数を先頭に出す**（取込前のスキップ件数の見当がつく） |
| 編集 | 行をタップ → ダイアログで `name`・`protein_amount` を直す |
| 削除 | 行をスワイプ → 確認のうえ削除 |
| 実装 | `from('foods').update(...)` ／ `.delete()`。**RPC は増やさない** |
| 検索 | 件数が増えたら要る `[仮]`。初期は一覧のみ |

**エラーは既存の ERR を使う。新設しない。**

| 事象 | ERR |
|---|---|
| `name` が空・100文字超 | `ERR-FOOD-003`（CSV取込と同じ検証） |
| `protein_amount` が負・数値でない | `ERR-FOOD-004`（同上） |
| 改名した先が既存と重複 | `ERR-FOOD-006`（`uq_foods_name` 違反・23505） |

- 検証規則は §3.4 と**同じ純関数**を通す。取込と編集で規則が割れないようにする。
- **削除しても取込済みの `meal_logs` は消えない。** `meal_logs` は栄養値を実体で持ち、`foods` を参照しない（ADR-0013）。
- したがって削除で過去の記録が壊れることは無い。

> ⚠️ 要確認（人間判断）: **`foods` は共通マスタで、認証済みなら誰でも編集・削除できる**（`../01_DB物理設計.md §3.4`）。
> 単一利用者では実害が無い。Phase2（複数利用者・NFR-SCALE-01）で見直す。
...
**食品マスタの編集・削除**（2026-08-22 追加・§7）

| TC-ID | 観点 | 期待 |
|---|---|---|
| TC-FEAT10-22 | 食品の `protein_amount` を編集 | 値が更新される。CSV の再取込では上書きされないままである |
| TC-FEAT10-23 | 食品を削除 | 行が消える。**その食品を使った過去の `meal_logs` は残る** |
| TC-FEAT10-24 | 改名して既存名と重複 | `ERR-FOOD-006`。`uq_foods_name` が弾く |

- 検証規則は取込（§3.4）と同じ純関数を通ることを確かめる。規則が2本に割れていないこと。
...
| 7 | ~~取込後の個別編集手段が無い~~（**解決**） | ~~誤った `protein_amount` を再取込しても直せない。訂正と削除の手段が無い~~ → **SCR-05 に食品一覧を置き、編集・削除できるようにする**（2026-08-22 岡田さん決定・§7） | — |
| 〃 | 〃 | 実装は PostgREST の `update` / `delete` のみ。**RPC は増やさない**（`foods` は共通マスタで RLS は `USING (true)`） | — |
| 〃 | 〃 | 一覧を持つことで**現行件数も見える**。取込前にスキップ予定件数を推し量れる（§10-11 の懸念も緩和） | — |
| 〃 | 〃 | 工数は W-10 に 0.5〜1日を加える（`../../70_見積・WBS.md`） | — |
