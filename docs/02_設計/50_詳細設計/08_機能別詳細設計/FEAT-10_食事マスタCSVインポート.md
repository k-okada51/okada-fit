...
| 項目 | 仕様 | 根拠 |
|---|---|---|
| 列構成 | `name`（食品名・text）／`protein_amount`（タンパク質量・float）の2列のみ | `foods` の列（`../01_DB物理設計.md §1.5`）。`id`/`created_at` はDB採番のためCSVに含めない |
| 列順 | ヘッダ名で解決（順不同可） | 列取り違え防止 |
| ヘッダ行 | **必須**（1行目）。`name,protein_amount` を正、`食品名,タンパク質量` をエイリアスとして受理 | Excel で人が作る想定。ヘッダ無しは列取り違えを検知できない |
| 区切り文字 | `,`（U+002C）のみ。TSV・セミコロン区切りは非対応 | RFC 4180 |
| 文字コード | UTF-8 ／ UTF-8(BOM付き) ／ Shift_JIS(CP932) の3種を受理し自動判定（§4-1）。判定結果は結果サマリに表示する | Excel 由来CSVは Shift_JIS・BOM付きUTF-8 が多い |
| 改行コード | LF / CRLF を受理。CR単独は非対応 | RFC 4180＋Unix系 |
| クォート | RFC 4180 準拠。`"` で囲んだセル内のカンマ・改行・`""`（エスケープ済み二重引用符）を許容 | |
| 最大行数 | 1,000行（ヘッダ除く） | NFR-PERF-05 の「数百件」の2倍を上限とし、超過は明示的に拒否 |
| 最大ファイルサイズ | 1 MiB（1,048,576 バイト） | 1行平均100バイト×1,000行＝約100KB。10倍の余裕。**上限の真の制約は RPC のペイロード上限**（下記） |
| `name` 長さ | 1〜100文字（前後空白トリム後） | |
| 空行 | 全列が空の行はスキップ（エラーにしない）。`skipped_count` に計上 | Excel 保存時の末尾空行対策 |

**ファイルサイズ上限の根拠**（旧構成では「Vercel のリクエストボディ上限を下回る」だった）:
...
検証は**全行を一括で実施し、DBを一切触らない**（2パス方式）。1行でも違反があれば RPC を呼ばず全件を返す。

| 項目 | 規則 | 違反時 |
|---|---|---|
| ファイル選択 | `file_picker` の結果が1件あること | ERR-FOOD-001 |
| 拡張子 | `.csv` であること | ERR-FOOD-008 |
| ファイルサイズ | ≤ 1 MiB | ERR-FOOD-003 |
| 文字コード | UTF-8 / UTF-8(BOM) / Shift_JIS のいずれかでデコード可能 | ERR-FOOD-005 |
| ヘッダ行 | 必須列 `name`・`protein_amount`（またはエイリアス）が揃う | ERR-FOOD-006 |
| 行数 | ≤ 1,000行（ヘッダ除く）かつ ≥ 1行 | ERR-FOOD-004 |
| 列数 | 各データ行はちょうど2列 | ERR-FOOD-002 `COLUMN_COUNT_MISMATCH` |
| `name` 必須 | 空文字・空白のみは不可 | ERR-FOOD-002 `EMPTY_NAME` |
| `name` 長さ | 1〜100文字 | ERR-FOOD-002 `NAME_TOO_LONG` |
| `name` 重複 | 同一ファイル内で `name` の重複を禁止（後勝ちで黙って捨てない） | ERR-FOOD-002 `DUPLICATE_NAME` |
| `protein_amount` 必須 | 空不可 | ERR-FOOD-002 `PROTEIN_EMPTY` |
| `protein_amount` 型 | 半角10進数のみ（`^-?\d+(\.\d+)?$`）。全角数字・桁区切りカンマ・単位付き表記は不可 | ERR-FOOD-002 `PROTEIN_NOT_NUMBER` |
| `protein_amount` 範囲 | 0 ≤ x ≤ 1000（`foods.protein_amount` の CHECK(≥0) と整合。上限は `[仮]`） | ERR-FOOD-002 `PROTEIN_NEGATIVE` / `PROTEIN_TOO_LARGE` |

**検証が端末側に移ったことの帰結**:

| 論点 | 内容 |
|---|---|
| 利点 | NG のとき通信が発生しない。応答が速い。サーバ側の実行時間上限を気にしなくてよい |
| 弱点 | 検証は**信頼境界の外**にある。改造したクライアントは検証を飛ばして RPC を呼べる |
| 緩和 | DB側の最後の砦は `foods.protein_amount` の CHECK(≥0) のみ。単一ユーザー運用のため当面は許容する `[仮]`（§10-1） |

## 4. 業務ロジック

**4-1. 文字コード判定**（`detectAndDecode`・純関数・NFR-QUAL-01）
...
`name` の先頭が `=` `+` `@` `TAB` `CR` のいずれかの行は、表計算ソフトへ再エクスポートした際に数式として解釈されうる（CSVインジェクション）。取込自体はブロックせず `warning_count` に計上し、画面に該当行を提示する `[仮]`。本アプリに CSV エクスポート機能が無いため実害は限定的（§10-6）。
...
- 呼び出しは `supabase.rpc('replace_foods', { p_rows })` の**1往復のみ**。1行1 INSERT だと数百往復となり NFR-PERF-05（≤5秒）を満たせない（§10-5）。
- 性能配分 `[仮]`: 端末側のデコード＋パース＋検証 ≤1,000ms（モバイル端末のため旧サーバ想定より緩める）／RPC 1往復（DELETE＋バルクINSERT）≤1,500ms／合計 ≤5秒（p95）。
...
| トランザクション境界 | `replace_foods()` の呼び出し1回＝1トランザクション。**検証は境界の外**（端末側・DB未接触）。関数内で失敗すれば自動ROLLBACKされ旧マスタが残る（NFR-MIGR-02 の可逆性）。部分コミットは発生しない |

## 6. エラー処理

HTTPステータスによる分類は廃止した。エラーの大半は端末内で確定するためである。RPC 由来の失敗は `PostgrestException` として返り、`foods_repository.dart` で ERR-ID にマッピングする。
...
- 行単位エラーの返し方は維持する。`line`（物理行番号・ヘッダ＝1）／`column`（`name` / `protein_amount` / `-`）／`reason_code` の3点で1件を特定する。画面表示は最大100件で打ち切る。
- ERR-FOOD-007 は「手動での再実行が有効」という意味で retryable=true とするが、**自動リトライはしない**（全置換の二重実行を避ける）。
- CSVの中身（食品名・数値）はログに残さない。ログ方針の正本は `../05_ログ設計.md`。Flutter 側のクラッシュ収集はスコープ外のため、端末ログは外部送信しない。

> ERRの完全列挙の正本は `../../60_テスト設計/02_RED母集合_受入基準・状態・エラー.md`（段6で集約）。本表はその入力とする。

## 7. 画面挙動・状態別表示

SCR-05 設定・プロフィール内の「食事マスタ取込」セクション。ファイル選択は `file_picker`（`FileType.custom` ＋ `allowedExtensions: ['csv']`）を使う。ドラッグ＆ドロップは iOS では使わないため考慮しない。

| 状態 | 表示 | 操作可否 |
|---|---|---|
| 初期/空 | `OutlinedButton`「CSVを選ぶ」＋説明テキスト（列構成・対応文字コード・上限行数/サイズ）。黄色の常時バナー（`Container` ＋ `Icon(Icons.warning_amber)`）で「取込は全置換であり既存マスタは消える」旨を掲示 | ファイル選択のみ可。[取り込む] は `onPressed: null` |
...
| 成功 | 緑の `SnackBar`（`ScaffoldMessenger.showSnackBar`）＋セクション内に結果サマリ `Card`（`imported_count` / `deleted_count` / `skipped_count` / `detected_encoding`）。`warning_count > 0` なら黄色 `Card` に該当行を併記 | 続けて別ファイルの取込が可能。FEAT-09 の表示は次回取得時に新マスタへ切替 |
...
- 全置換の確認ダイアログには削除予定件数を出したいが、現行件数を取る手段を本書の範囲に持たないため件数は出さない（§10-7）。
- 文字コードの誤判定に気づけるよう、成功サマリの `detected_encoding` は必ず可視化する。
- `name` はプレーンテキストとして `Text` ウィジェットに描画する。Flutter は HTML を解釈しないため、XSS は構造的に発生しない（§10-6）。
...
| # | 論点 | 内容 | 重大度 |
|---|---|---|---|
| 1 | `foods` に `user_id` が無い | 本機能はユーザー個別ではなく**全ユーザー共通のマスタを破壊的に置換**する。単一ユーザー運用（NFR-SCALE-01 でマルチテナントは適用外）では成立するが、Phase2 のマルチユーザー化で即破綻する。加えて `user_id = auth.uid()` 相当のRLSが構造的に張れない。さらに端末から `security definer` の `replace_foods()` を直接呼ぶ構成のため、**認証済みユーザーなら誰でも全件置換できる**。Phase2 では関数内での権限チェックが要る | 🔴 高 |
| 2 | `foods.name` に UNIQUE が無い | upsert のキーが存在しないため取込モードは全置換しか選べない。全置換は `foods` を一度空にするため、`DELETE` と `INSERT` が別トランザクションになると FEAT-09 の候補提示が一瞬空になる。本書は `replace_foods()`（DB関数）で単一トランザクション化して回避したが、これは**DBオブジェクトの追加**であり `../01_DB物理設計.md` に無い。`foods.name` に UNIQUE を張って upsert する案もあるが、列制約の追加は本書の裁量外（§10-12） | 🔴 高 |
| 3 | `protein_amount` の単位が未定義 | 「1食分あたり」か「100gあたり」かが `../01_DB物理設計.md §1.5`・`01_データモデル.md` のどこにも無い。CSVの中身を決める本書が単位を確定しないと、同じ数値が別の意味で混在し、FEAT-09（RULE-005）の「△△を食べると補える」が成立しない。本書は §4-2 で「1食分あたり」を `[仮]` 採用した | 🔴 高 |
| 4 | 文字コードの誤判定 | Shift_JIS デコーダはほぼ任意のバイト列を受理するため、判定順序を誤ると文字化けが**エラーにならず静かに取り込まれる**。§4-1 の順序（BOM→UTF-8厳密→Shift_JIS）と `detected_encoding` の可視化で緩和したが、CP932 の機種依存文字など完全ではない。取込前プレビュー（先頭数行を画面で確認させる）を追加するか要判断 | 🟡 中 |
| 5 | 性能（NFR-PERF-05） | 1行1 INSERT を `supabase_flutter`（PostgREST）で行うと数百HTTP往復となり、モバイル回線では1往復50〜200msで 15〜60秒かかり ≤5秒を満たせない。**バルクINSERT（RPC1往復）が必須要件**であり、実装時に個別INSERTへ退化しないよう TC-FEAT10-13 で担保する | 🟡 中 |
| 6 | CSVインジェクション・DoS | `=` 始まりセルは本アプリにエクスポート機能が無いため直ちに実害は無い。XSS は Flutter が HTML を解釈しないため構造的に発生しない（旧構成では React の既定エスケープに依存していた）。一方、検証が端末側に移ったことで**改造クライアントは検証を飛ばして RPC を直接呼べる**。巨大ペイロードによる DoS も同様。DB側の防御が CHECK(≥0) だけでよいかが未定（NFR-SEC-04/05） | 🟡 中 |
| 7 | 取込後の個別編集手段が無い | 取り込んだ食品を1件だけ直す／消す手段が要件にも本書にも存在しない。1件の誤りを直すのに毎回CSV全体を作り直して全置換する運用になる。現行件数を取る手段も定義していないため、取込前の確認ダイアログに削除予定件数を出せない（§10-11・§10-12 と関連） | 🟡 中 |
| 8 | エラー詳細が共通契約に無い | 行単位エラー（`line`/`column`/`reasonCode`）は `02_API設計.md §5` の共通エラー応答契約（`error_code`/`message`/`retryable`）に定義が無い。本構成では通信に載らず端末内で完結するが、共通のエラー表示部品と構造が揃わない点は変わらない。段3の契約改訂で整理するのか、本機能限定の例外とするのかを決める必要がある | 🟡 中 |
| 9 | 追加依存とプラグイン挙動 | Shift_JIS(CP932) のデコーダは **Dart 標準に無い**。`charset_converter` 等のプラグインが要る。プラグインは iOS ネイティブ実装に依存するため、実機での挙動確認が必須。代替として Dart 純実装の変換表を自前で持つ案もあるが保守コストが上がる（NFR-MAINT-01） | 🟡 中 |
| 10 | 上限値の根拠が薄い | 1,000行／1 MiB／`name` 100文字／`protein_amount` ≤1000 はいずれも `[仮]`。岡田さんの手元CSVの実サイズを確認して確定させる。**RPC のペイロード上限**（Supabase / PostgREST 側の制約）は未確認のため、1,000行分の JSON が通ることを実測で裏取りする | 🟢 低 |
| 11 | 全件ロールバックか部分成功か | 利用者から「1件失敗したら全件ロールバックだと思うが、失敗したものだけスキップして続行できないか」との質問。**技術的には可能**。実現方法とトレードオフは §10.1-A に整理した。今回は方式を変更しない（現行＝全件ロールバック） | 🟡 中 |
| 12 | 全置換ではなく1件追加ができないか | 利用者から「全置換とあるが1件追加はできないか。名前で比較して既存はスキップし重複を防ぎたい」との質問。**根本原因は `foods.name` に UNIQUE が無いこと**。DB側で名前の重複判定を担保できない。UNIQUE を足せば実現できる（§10.1-B）。`../01_DB物理設計.md` の改訂を伴うため本書では決めない | 🔴 高 |
| 13 | 上位文書との乖離 | 本書は Flutter + Supabase 構成で記述している。段3（`02_API設計.md`）は `/api/*` の Route Handler 契約のままで、`POST /api/foods/import`（multipart）が残っている。契約表からの削除と RPC 契約の追記が要る | 🔴 高 |
