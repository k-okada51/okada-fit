...
| # | ロジック | 定義 | 対応 |
|---|---|---|---|
| L1 | 目標タンパク質量 `target_g` | RULE-001（体重×2g）。**算出の正本は FEAT-07** の Dart 純関数を呼ぶ。本機能では再実装しない（確定・§10-12） | RULE-001 / FEAT-07 |
...
**目標値と達成率は Dart が計算する**（2026-08-08 確定・E群D）。

| 事項 | 内容 |
|---|---|
| RPC が返す値 | `weight_kg`・`intake_g`。どちらも素の値である |
| RPC が返さない値 | `target_g`・`rate_pct`。計算済みの値は返さない |
| 算出の場所 | `app/lib/domain/nutrition.dart`（L1）と `app/lib/domain/gauge.dart`（L3・L4） |
| SQL に書かないもの | `weight_kg * 2`・達成率の除算・小数第1位の丸め |
| 正本 | 算出式は FEAT-07 §4.5。本書では再定義しない |
...
- **ゲージの目標値と達成率は Dart が計算する**（確定・§4）。RPC の戻り値をそのまま出す項目ではない。
- 体重・摂取量は RPC の値をそのまま使う。計算は `nutrition.dart` と `gauge.dart` に閉じる。
