...
**回数・重量は記録しない**（2026-08-22 岡田さん決定・ADR-0008 を再確認）。**設計が正である。**

要件定義（`01_要件定義/02_機能要件/`）の FEAT-04 概要と REQ-03 は回数・重量を求めたままだが、**文言のほうを実態に合わせて直す**（§10-10）。設計を要件に合わせる改訂は行わない。
...
**操作1（`p_menu_ids`）は FEAT-03 の提案結果をそのまま受けられる**（ADR-0021・2026-08-08）。

| FEAT-03 が返すもの | 本機能での扱い |
|---|---|
| `menu_id` | `p_menu_ids` の要素になる |
| `order`（実施順） | 配列の**並び順**として渡す。列としては持たない |
| `reason` | 渡さない。**保存しない** |

- FEAT-03 は登録済みの種目から選ぶため、`menu_id` は**必ず本人の `training_menus` に実在する**。
- そのため §3.1 の所有検証（`ERR-TRAINING-003`）は、FEAT-03 経由では通常ヒットしない。**手動選択の経路のために残す。**
- 重複も FEAT-03 側で除かれる。`uq_tsd_session_menu` との整合は保たれる。
- 実施順を保存しないのは、記録の目的が「何をやったか」であり順序ではないため（ADR-0008 と同じ理由）。
...
`is_done` は本PJで**唯一の永続状態**（ST-01（未実行）/ST-02（実行済））。FEAT-05 ダッシュボードのヒートマップ（実施有無）と種目名ツールチップの集計元になる。

### 入館記録の扱い

入館記録（`gym_visits`）は同じ SCR-03（トレーニング）から行う。ただし `training_sessions` とは独立に扱う。
...
> ~~⚠️ 要確認（人間判断）: T02 の更新方式が段3の API 契約に無い。次の2案から確定する。~~（**解決**・2026-08-08）
>
> | 項目 | 内容 |
> |---|---|
...
>
> **段3は改訂済み。案Aである。** 同 §3 に `from('training_session_details').update({is_done:true})` がある。
> 状態ガード `.eq('is_done', false)` 付き（ADR-0008）も同 §3 に明記された。
...
| 10 | 要件定義の記述が実態と合っていない（**方針は決定済み**） | ~~どちらが正か未定だった~~ → **記録しない方針で確定**（2026-08-22 岡田さん決定）。**設計が正**であり、要件の文言を実態に合わせて直す。残る作業は要件ブランチでの文言修正だけで、**設計側の変更は無い**。差分は下表 | 🟢 低 |
...
> ~~⚠️ 要確認（人間判断）: #2 #3 #4 #6 #7 は正本側の改訂なしに本書の `[仮]` を確定にできない。~~（**#4 #7 のみ解決**・2026-08-08）
> ~~改訂先は `../../30_データ・IF設計/03_ドメインイベント.md` または `../../30_データ・IF設計/02_API設計.md`。~~
> ~~特に #4 は FEAT-05 の集計仕様と直結する。FEAT-04/FEAT-05 をまたいで1つの結論に揃える必要がある。~~
> **#4 #7 は段3で解決済み。** `../../30_データ・IF設計/02_API設計.md §3` が `gym_visits` を「ヒートマップの集計には使わない」と明記した。
> 塗り条件は同 §4.3 で `training_session_details.is_done` が1件以上 true の日に確定した。
> #7 の PostgREST 直接 update も同 §3 に載っている。

> ⚠️ 要確認（人間判断）: #2 #3 #6 は残る。改訂先は `../../30_データ・IF設計/03_ドメインイベント.md`。
> #3 は T02 の逆遷移（T03相当）が同 §2 の一覧・§3 の遷移図のどちらにも無い。
> #6 は同 §2 の T01 が「明細を `not_done` で登録」のままである。
> 段3 §3 の `p_is_done` を作成時に `true` で受ける扱いが書かれていない。
> #2 は同一日に複数セッションを許すかの業務判断。制約を足すなら `../01_DB物理設計.md` にも及ぶ。
