...
> **とくに FEAT-03 は ADR-0001 が根拠ADRそのもの**である。EXT-01 の実体・モデル選定・フォールバック方針・コスト按分タグが全て ADR-0001 に依存していた。Gemini API 直接呼び出しへの切替は ADR-0001 の前提を丸ごと置き換えるため、**本機能の設計を確定させる前に後継ADR（Gemini API 直接呼び出し・モデル選定・フォールバック不在の受容）を起票すること**。後継ADRが無い限り本書 §3(2)・§6・§10 #6 は根拠を持たない。
...
2層で定義する。(1) が Flutter から見える契約、(2) が Edge Function 内部の実装仕様。

**(1) Flutter → Edge Function（呼び出し契約）**
...
- 旧契約 `POST /api/menus/generate`（段3 `02_API設計.md §4.2`）は**廃止**。Edge Function `generate-menu` に置き換わる。段3の契約表の改訂が要る（§10 #12）。

**(2) Edge Function → Gemini API（EXT-01・実装仕様）**
...
| 応答検証 | パース結果を zod（Deno/TS）で検証。失敗は ERR-MENU-004 | §3.1 |
...
> ⚠️ 要確認（人間判断）: ERR-AI-CREDIT と ERR-AI-RATE の切り分け条件。Gemini API はクォータ超過も一時的レート超過も 429 `RESOURCE_EXHAUSTED` を返し得るため、HTTP ステータスだけでは分離できない `[仮]`。エラーボディの `status`／`reason` で判定するか、両者を1つに統合するかを決める。
...
> ⚠️ 要確認（人間判断）: #6 フォールバック不在を受容するか、Edge Function 内に自前のモデル切替を実装するか。受容する場合は NFR-AVAIL-05 の縮退範囲に「FEAT-03 全停止」を明記する。
