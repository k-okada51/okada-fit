...
~~逆向きの経路も1つある。FEAT-03 の提案から［登録］された種目も本機能が作る（§3.3）。~~（**2026-08-08 改訂**・ADR-0021）
**FEAT-03 は種目を登録しない。** `training_menus` に行を作るのは本機能だけである。
...
**`how_to` は種目の登録と同時に入れる運用とする**（2026-08-08 確定・ADR-0021）。

| 項目 | 扱い |
|---|---|
| 入力のタイミング | 種目を登録する時（C-06）。器具登録の流れの中で一緒に入れる |
| 必須か | **任意。空を許容する** |
| 空だった場合 | FEAT-03 の結果表示で**説明が空欄になる**。それを許容する |
| AI に書かせるか | **書かせない。** 幻覚の余地を戻さないため |

- `how_to` は FEAT-03 が今日のメニューを表示する際の説明文になる（FEAT-03 §7）。
- 後から埋めたい場合は種目の編集（C-08）で追加する。
...
### 登録順序の依存を隠す（確定・2026-08-22）

器具登録には `gyms` と `training_menus` が先に要る（`gym_id` は NOT NULL FK、種目は1件以上）。
**画面遷移させずに、同じ画面のダイアログで作らせる。**

| 要素 | 挙動 |
|---|---|
| ジム | ドロップダウン ＋ **［＋新しいジム］**。ダイアログで名前を入れて即時作成し、選択状態にする |
| 種目 | チェックボックス一覧 ＋ **［＋新しい種目］**。ダイアログで名前・部位・やり方を入れて即時作成し、選択状態にする |
| 登録ボタン | ジム未選択・種目0件のときは**非活性**（ERR-MACHINE-003 を出す前に押させない） |

- **RPC に種目・ジムの作成を含めない。** `create_machine` の引数は既存のままとする。
- 種目・ジムの作成は**別呼び出し**（PostgREST insert）。器具の登録とは別トランザクションになる。
- **受容したリスク**: 種目を作った直後に器具登録が失敗すると、**種目だけが残る**。
- 実害は小さい。残った種目は次回そのまま選べる。孤児にはならない。
- `how_to` は種目作成のダイアログで一緒に入れる。空も許容する（ADR-0021・§3.3）。
...
| TC-FEAT01-24 | **欠番。** AI提案からの種目登録は廃止（ADR-0021。FEAT-03 は種目を登録しない） | — |
...
| # | 論点 | 内容 | 重大度 |
|---|---|---|---|
| 1 | ~~登録順序の依存が利用者に露出する~~（**解決**） | ~~種目・ジムの作成まで同じ RPC に含めるかが未決だった~~ → **含めない**（2026-08-22 岡田さん決定）。SCR-02 内のダイアログで種目・ジムを即時作成し、**RPC は器具の3本のまま**とする | — |
| 〃 | 〃 | 画面遷移は発生しない。利用者から見て順序の依存は隠れる（§7） | — |
| 〃 | 〃 | **受容したリスク**: 「種目を作ってそのまま器具を登録」は2呼び出し＝2トランザクションになる。中間で失敗すると**種目だけが残る** | — |
| 〃 | 〃 | 実害は小さい。残った種目は次回の登録でそのまま選べる。孤児レコードにはならない（`training_menus` は単独で意味を持つ） | — |
...
> ~~⚠️ 要確認（人間判断）: 段3 の HTTP 契約は本書の方式に置き換わる。段3側の改訂が要る。~~（**解決**・2026-08-08）
>
...
>
> **段3は改訂済み。** `../../30_データ・IF設計/02_API設計.md §2・§3` が本書の方式になっている。
> 器具は RPC `create_machine` / `update_machine` / `delete_machine`。
> 器具一覧・種目・ジムは PostgREST `training_machines`・`training_menus`・`gyms`。
> 旧 `/api/*` は同 §3 末尾「旧契約との対比」に対応表として残るだけである。
> 共通エラーは同 §5.1 で経路が2つに分かれた。PostgREST・RPC は同 §5.4 の SQLSTATE 写像になる。
> その写像の正本が `../07_実装共通設計パターン.md §1` であることも同 §5.4 に明記された。
