...
| 項目 | 内容 |
|---|---|
| キャッシュ単位 | 部位をキーに Flutter 側で保持する |
| 効果 | 同一部位の再選択では再取得しない（NFR-PERF-02 の体感短縮） |
...
| # | 論点 | 内容 | 重大度 |
|---|---|---|---|
| 1 | 該当器具0件のときの挙動 | 「例外にせず空配列」を確定案とした（0件は正常系）。残る判断は SCR-03 の導線で、器具0件でも FEAT-03 へ進めてよいか。器具が空だと自重種目しか出ず EXT-01 の課金だけ発生するため[メニュー生成]非活性を`[仮]`とする | 🔴 高 |
