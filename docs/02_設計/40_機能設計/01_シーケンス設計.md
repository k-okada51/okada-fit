---
status: draft
---

# 機能設計（FEAT別シーケンス） — okada-fit

> **目的**: 各機能要件（FEAT）の処理シーケンス（画面操作→サーバ→AI/DB→応答）を定義し、実装の振る舞いを固定する。API契約は `../30_データ・IF設計/02_API設計.md`、データは `01_データモデル.md`、状態は `03_ドメインイベント.md` を正本とし、本書はその**流れ**を担う。
> **書き方**: 実データは書かず、登場アクター（Flutter/Edge Function/Gemini API/Supabase）と主要メッセージ・分岐（正常/エラー/縮退）を Mermaid sequenceDiagram で示す。エラーは `02_API設計.md §5` の共通契約、状態は ST-ID を参照。

> ⚠️ **本書はたたき台（2026-07-25 生成）**。画面モック（SCR-01〜05）の人間承認と併せて確定する。

> 📖 ID（`FEAT-` `NFR-` `RULE-` 等）の意味は [ID早見表](../00_ID早見表.md) を参照。

## 目次
1. [FEAT-08 食事撮影・タンパク質計算](#1-feat-08-食事撮影タンパク質計算)
2. [FEAT-03 AIメニュー提案](#2-feat-03-aiメニュー提案)
3. [FEAT-05 ダッシュボード表示](#3-feat-05-ダッシュボード表示)
4. [FEAT-02 部位→器具の絞り込み](#4-feat-02-部位器具の絞り込み)
5. [FEAT-09 タンパク質残量・不足分提示](#5-feat-09-タンパク質残量不足分提示)

## 1. FEAT-08 食事撮影・タンパク質計算
対応画面: SCR-04 食事記録 ／ 写真は非保存（ADR-0003）
API: `functions.invoke('analyze-meal')`（EXT-01 Google Gemini API）／ `from('meal_logs').insert(...)`

```mermaid
sequenceDiagram
  actor U as 岡田さん
  participant F as Flutter(SCR-04)
  participant E as Edge Function(analyze-meal)
  participant G as Gemini API(EXT-01)
  participant P as Supabase(PostgREST)

  U->>F: 📷 撮影（image_picker・ImageSource.camera）
  F->>F: プレビュー＋長辺1024pxにリサイズ
  Note over F: この時点では未送信（自動送信しない）
  U->>F: [解析する]
  F->>F: base64 化（Storage は経由しない・ADR-0003）
  F->>E: functions.invoke('analyze-meal')（画像を直接POST）
  E->>E: Supabase Authで本人確認（JWT検証）
  E->>G: generateContent（画像＋response_schema, gemini-3.7-flash）
  alt 成功
    G-->>E: {food_name, dish_names[], kcal, protein, sugar, fat}
    E-->>F: 栄養4項目を返す（画像は破棄）
    F->>U: 📊 栄養価を表示＋[記録する]
    U->>F: [記録する]
    F->>P: from('meal_logs').insert（栄養4項目・画像カラムなし）
    P-->>F: 保存OK
    F->>U: SnackBar「記録しました」＋残量再計算
  else AI失敗（402/429/timeout）
    G-->>E: エラー
    E-->>F: ERR-AI-*（§API設計§5）
    F->>U: SnackBar「解析に失敗しました」（記録・閲覧は継続・NFR-AVAIL-05）
  end
```

- 画像は Edge Function へ直接POSTする。Supabase Storage には置かない（ADR-0003）。
- Edge Function は業務テーブルに触れない。保存は Flutter から PostgREST で行う。
- ローディング表示は最大20秒想定（NFR-PERF-04）。非冪等・自動リトライなし。
- 撮影→プレビュー→[解析する]→[記録する] の2ステップで、失敗写真の無駄な課金を防ぐ。
- `meal_logs.eaten_date` は**端末の時刻で決めて渡す**（2026-08-08 確定・ADR-0014）。
- 記録するのは栄養4項目のみ。日次合計は `SUM(protein_g)` で導出する（列で持たない・ADR-0013）。
- 解析結果は表示のみで**手修正しない**（ADR-0015）。操作は［記録する］と［撮り直す］の2つ。
- 手入力の経路も持たない。AI が失敗した食事は**記録できない**（撮り直すか諦める）。

## 2. FEAT-03 AIメニュー提案
対応画面: SCR-03 トレーニング
API: `from('training_menus').select(...)`（器具取得）／ `functions.invoke('generate-menu')`（EXT-01 Google Gemini API）

```mermaid
sequenceDiagram
  actor U as 岡田さん
  participant F as Flutter(SCR-03)
  participant P as Supabase(PostgREST)
  participant E as Edge Function(generate-menu)
  participant G as Gemini API(EXT-01)

  U->>F: 部位を選択（胸/背中/脚/肩/腕）
  F->>P: from('training_menus').select（埋め込み select・body_part=胸）
  Note over F,P: 種目→中間(machine_menus)→器具(training_machines) の3ホップ（AI不使用・RULE-004）
  P-->>F: 種目行＋ネストした中間行・器具行
  F->>F: 器具IDで畳み込み（DISTINCT 相当の重複排除）
  F->>U: 器具を表示
  U->>F: [メニュー生成]
  F->>E: functions.invoke('generate-menu')（body_part, machine_ids[]）
  E->>P: 器具の実在・所有・部位整合を検証（RLS・呼び出し元JWT）
  P-->>E: 器具×種目の行（menu_id を含む）
  E->>E: menu_id で畳んで候補の種目一覧にする
  E->>G: generateContent（種目一覧＝menu_id つき・response_schema）
  Note over E,G: 指示は「この中から今日やる分を選び、実施順を決めよ」
  G-->>E: {menus:[{menu_id, order, reason}]}
  E->>E: menu_id が候補集合に含まれるか検証（違反は ERR-MENU-007）
  E-->>F: 今日のメニュー（menu_id・実施順・理由）
  F->>U: 実施順つきで表示（種目名・やり方は DB の値）
```

- **FEAT-03 は「登録済みの種目から、今日やる分を選んで順番を決める」**（ADR-0021）。
- AI へ渡すのは `menu_id` つきの種目一覧。返るのは `menu_id`・実施順・理由だけ。
- **種目名とやり方は AI に書かせない。** `training_menus` から引いて画面に出す。
- 器具↔種目は多対多（中間テーブル `machine_menus`）。1台の器具が複数部位に対応する。
- 絞り込みは3ホップ。器具の重複排除（`DISTINCT` 相当）が必ず要る（§4）。
- 生成前の検証は「器具が指定部位の種目を1つ以上持つ」で判定する（部位の完全一致ではない）。
- 検証NGなら Gemini API を呼ばない。無駄な課金を出さない。
- **幻覚は構造的に起きない。** AI は入力で渡した `menu_id` の集合からしか選べない。
- 集合外の `menu_id` が返れば異常として弾く（`ERR-MENU-007`・502）。
- 選ばれた `menu_id` はそのまま `training_session_details` の明細になる（FEAT-04）。

## 3. FEAT-05 ダッシュボード表示
対応画面: SCR-01 ダッシュボード ／ API: RPC `get_dashboard`（引数は `p_period` / `p_today` / `p_range_start` / `p_range_end` / `p_month_start` / `p_month_end`）

```mermaid
sequenceDiagram
  actor U as 岡田さん
  participant F as Flutter(SCR-01)
  participant R as Supabase(RPC get_dashboard)
  participant D as PostgreSQL(RLS)

  U->>F: ダッシュボードを開く
  F->>F: 当日と表示範囲を決める（端末のタイムゾーン・ADR-0014）
  F->>R: supabase.rpc('get_dashboard', {'p_period': 'month', 'p_today': 当日, 'p_range_start': 範囲開始, 'p_range_end': 範囲終了, 'p_month_start': 今月初日, 'p_month_end': 今月末日})
  R->>D: 当日摂取(meal_logs SUM)・目標(体重×2g)・今月の実施日数・実施日を集計
  Note over R,D: 塗る日＝training_session_details.is_done が1件以上 true の日（gym_visits は使わない）
  D-->>R: 集計結果
  R-->>F: {protein_gauge, training_count, heatmap}
  F->>U: CircularProgressIndicator(当日の達成率・100%頭打ち)＋「今月 N / 12 回」＋Heatmap(実施有無・種目名)
  U->>F: 期間切替(SegmentedButton 日/週/月)
  F->>R: supabase.rpc('get_dashboard', {'p_period': 'week', 'p_today': 当日, ...})
  Note over F,R: 変わるのはヒートマップの表示範囲だけ。ゲージは常に当日
```

- 初期表示 ≤2秒（NFR-PERF-01）。チャートは `fl_chart` で描画する。
- 集計はRPC1回で完結する。Flutter からの往復は期間切替のたびに1回。
- **ゲージ（`protein_gauge`）は常に当日**（2026-08-08 確定）。`p_period` の影響を受けない。
- 期間切替が変えるのはヒートマップの表示範囲だけ。週・月の平均や累積は取らない。
- **ヒートマップを塗る条件は `training_session_details.is_done` が1件以上 true**（2026-08-08 確定）。
- `training_sessions` の行があるだけでは塗らない。予定だけで未実施の日は塗らない。
- `gym_visits`（入館履歴）は使わない。ST-02（実行済）を持つ意味がここで生きる。
- 戻り値の `training_count` は「今月の実施日数（`done_days`）／目標回数（`target`）」。
- `done_days` はヒートマップと同じ条件で数える。`target` は `users.target_training_count`（既定12・RULE-007）。
- 体重が未設定なら `protein_gauge` は `null`。エラーにせず入力を促す表示に切り替える。
- 「当日」と「今月」の範囲は端末の時刻で決める（ADR-0014・`../50_詳細設計/07_実装共通設計パターン.md §2`）。

## 4. FEAT-02 部位→器具の絞り込み
対応画面: SCR-02 器具登録／SCR-03 トレーニング ／ API: `from('training_menus').select(...)`（AI不使用の決定的処理・RULE-004）

```mermaid
sequenceDiagram
  actor U as 岡田さん
  participant F as Flutter(SCR-02/SCR-03)
  participant P as Supabase(PostgREST)
  U->>F: 部位を選択（SegmentedButton・5値）
  F->>F: 部位を検証（enum 5値・単一選択）
  F->>P: from('training_menus').select（埋め込み select・body_part=脚）
  Note over F,P: training_menus → machine_menus → training_machines の3ホップを1往復で取得
  P-->>F: 種目行＋ネストした中間行・器具行（0件を含む）
  F->>F: 器具IDで畳み込み（DISTINCT 相当の重複排除）
  F->>U: 絞り込み結果を表示（≤1秒・NFR-PERF-02）
```

- 器具↔種目は多対多。中間テーブル `machine_menus` を必ず経由する（**3ホップ**）。
- **`DISTINCT` が必須**。1台が同じ部位の種目を複数持つと、付けないと同じ器具が重複して返る。
- 埋め込み select 自体は `DISTINCT` を持たない。同じ役割を Flutter 側の畳み込みが担う。

## 5. FEAT-09 タンパク質残量・不足分提示
対応画面: SCR-01 ダッシュボード／SCR-04 食事記録 ／ API: `supabase.rpc('get_protein_remaining', ...)`（AI不使用・DB照会）

```mermaid
sequenceDiagram
  actor U as 岡田さん
  participant F as Flutter(SCR-01/SCR-04)
  participant R as Supabase(RPC get_protein_remaining)
  participant D as PostgreSQL(RLS)
  F->>F: 集計対象日を決める（端末のタイムゾーンの当日・ADR-0014）
  F->>R: supabase.rpc('get_protein_remaining', {'p_target_date': 当日})
  R->>D: 目標(体重×2g)−当日摂取(meal_logs SUM)＝残量、foodsから不足を補う候補を照会
  D-->>R: {target_g, intake_g, remaining_g, suggestions}
  R-->>F: 残量＋補う食品候補（RULE-005・AI不使用）
  F->>U: 「あと○g・△△を食べると補える」
```

- 残量が0なら `suggestions` は空配列。候補0件でもエラーにしない。
- SCR-04 で記録した直後にも同じRPCを呼び直す（§1 の残量再計算）。
- **集計対象日は端末の時刻で決める**（2026-08-08 確定・ADR-0014）。RPC 内で `CURRENT_DATE` を使わない。
- 残量が0に戻る（リセットされる）タイミングも端末時刻の日付変更に従う。
- 横断方針の正本は `../50_詳細設計/07_実装共通設計パターン.md §2`。
- `foods.protein_amount` は**1食分あたり**のため、残量(g)と直接比較できる（ADR-0012）。

> 関連: API契約＝`../30_データ・IF設計/02_API設計.md` / データ＝`../30_データ・IF設計/01_データモデル.md` / 状態＝`../30_データ・IF設計/03_ドメインイベント.md` / 画面モック＝`../../01_要件定義/02_機能要件/02_画面/`（SCR-01〜05・人間承認ゲート）。
