...
| 論点 | 採用 | 根拠 |
|---|---|---|
| アプリ形態 | Flutter アプリ（iOS先行・TestFlight配布）＋ Supabase `[仮]` | 個人利用・小規模（DEC-D01・ADR-0010 で確定） |
...
| データ永続化 | 無料枠のDB（Supabase/PostgreSQL）に集約。本人分離はRLS | DEC-B03・ADR-0004/0005 |
...
3. 初期設定: 体重入力 → PostgREST で `users` に保存。必要タンパク質量（体重×2g）は Dart 純関数と SQL 関数 `calc_target_protein_g` で算出する。食事マスタは RPC `import_foods` で初期取り込み。
...
| FEAT-07 必要量算出 | Dart 純関数＋SQL関数 | `calc_target_protein_g` |
